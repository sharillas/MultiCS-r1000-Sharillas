...
- v1.21 (fixes produção + CAIDs PT/13E/19.2E): **fix crítico dcwtimeout()** — TIMING ativo sem MINPERIOD/FRACTION devolvia timeout 0 e TODOS os ECMs falhavam em ~10ms (guard no código: nunca devolve 0, budget só com período observado; config de produção agora define `DEFAULT TIMING MINPERIOD/FRACTION`); **fix recvtime no RETRY** (CCcam/Newcamd/Mgcamd/Camd35/Cs378x — re-pedidos do mesmo canal falhavam instantaneamente com o timeout antigo); **fix SIGFPE (divisão por zero)** em `srv_healthscore` com ecmnb=0 (crash-loop em produção a cada ~15s com o browser na GUI) e em 2 células da GUI (coluna Cards e secção ECM Dedup com contadores a zero); **fix spin 100% CPU no /dashboard** (walks da lista circular `ecmdata` sem guard — cada autorefresh criava uma thread infinita); **nova opção `{ nocheck=yes }` nos readers** (topologia cliente+readers no mesmo IP externo, ex: 203.0.113.10 — a proteção anti-circular já não bloqueia a partilha); **identificações CAID oficiais** aplicadas em profiles/CCcam.providers/CCcam.channelinfo/cards GUI: MEO 1814 (005211/005221/000007), NOS 1802 (004801/004901), Canal+ Polónia 1813:000068, Tivùsat 183D/183E/1856, Polsat Box 1803, packages 19.2E (Movistar+, Canal+ France, TNT SAT 1818, Sky DE 098C/098D, HD+ 1830-188A, Canal Digitaal, TV Vlaanderen, Austriasat) + listas completas de canais; **parser de CCcam.providers aceita o formato SRVID `CAID:PROVID|NOME`** (antes só 8hex "nome" — idents nunca apareciam na GUI); **coluna Cards/Providers com ident a 6 dígitos + nomes de ident** (ex: `1814: 005211 MEO ID (ident real transmissao)`); **perfis internos DEFAULT/BISS Emu escondidos** da lista de profiles da GUI; **profiles reescritos só com os packages ativos 30W/13E/19.2E** (eliminados Sky UK/IT, Viaccess/SECA genéricos, Conax, CryptoWorks, Irdeto, BetaDigital, Bulsatcom, Turksat...); Softcam.cfg com a lista dos 3 satélites (BISS Abertis/EBU/feeds; chaves antigas do updater preservadas comentadas); cards da GUI com **ident prioritário sem duplicação** de nomes (1884/0500:050800/186A:003411 corrigidos)
- v1.22 (observabilidade + dashboard): **sessões GUI persistentes** (ficheiro `/var/tmp/multics.sessions` — o browser deixa de ir ao login após restarts); **log em ficheiro com rotação** (`-f` no serviço: `/var/tmp/multics.log` → `.1` aos 5MB); **NOK cache por reader+canal** (~8s — um reader que responde NOK num canal é saltado nos zappings seguintes, só quando há alternativa — menos tráfego NOK); **página Packages** (dashboard por satélite: package, CAID:Ident, perfil ligado, Ecm OK, readers que servem — dados das listas oficiais); **página Test Channel** com formulário (ativar na sessão, gravar no multics.cfg, desativar); **crash handler relança o sinal** (gera core dump para análise post-mortem — core_pattern `/var/tmp/cores` + serviço `multics-crashreport` que regista o backtrace gdb em `/var/tmp/crash_reports.log`); **deploy.ps1** (build → pscp → restart → verifica) e **GitHub Actions CI** (build musl em cada push)
- v1.23 (segurança + UX): **login guard anti brute-force** (5 falhas → bloqueio 30s por IP; tabela de 64 IPs, throttle global contra rajadas, `HTTP LOGIN MAXFAIL`/`HTTP LOGIN LOCKTIME` configuráveis, reset no sucesso — testado em produção); **sessões**: botão "Terminar todas as sessões" na Configs + invalidação automática quando a password muda (editor/upload); **responsividade mobile** (tabelas com scroll <900px); **fuzz HTTP** (`tests/fuzz_http.sh` — métodos/paths/headers/POSTs malformados, zero crashes); **ip2country diário iplocate** (loader aceita CIDR `NETWORK/PREFIXO,CC` e CSV sem aspas — 774k redes IPv4, bandeiras na GUI; ferramenta `tools/update_ip2country.py` com backup `.bak-<data>` e sugestão de cron); limpeza do repo (código morto r82 removido: twin/ipenc/iplock/mgcamd-conn/Makefile...; segredos fora do repo via `deploy.secrets.ps1` gitignored)

## Roadmap v1.31 (estudo MEO+NOS, set 2026) - pendente de implementacao
Base do estudo: linha directa MEO (12700) + circuito, capturas cwl/dcwlog (v1.30.1).
Conclusoes MEO: criptoperiodo 15s, alternancia CW0/CW1 perfeita na directa,
overhead do rele MultiCS ~10ms; o circuito esticava 32-60s por fontes sujas.

1. Perfil com SERVERS: id1,id2 (lista explicita de readers por perfil;
   precedencia sobre profiles=; readers sem profiles= deixam de servir tudo)
2. Quarentena por qualidade de CW: cwbad (nagra/cwlr/cwpk/stale) + FEEDBACK DO
   CLIENTE (re-pedido do mesmo hash N segundos apos entrega = a CW falhou)
   -> suspende o reader N minutos. Sem box nossa, a box do cliente e o sensor.
3. Ident whitelist por reader (replicar o "so 5211" do colega; cortar lixo 000007)
4. Marcacao hop1/hopN nos readers (directa vs circuito)
5. CYCLE_CHECK com o modelo real (15s + alternancia estrita) para detectar
   desync de fonte (o circuito entrega par completo novo = lixo)

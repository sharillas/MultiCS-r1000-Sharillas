...
wget https://github.com/sharillas/MultiCS-r1000-Sharillas/releases/download/v1.43/multics-r1000-v1.43.tar.gz
# 2. extrai e instala
tar xzf multics-r1000-v1.43.tar.gz
cd multics-r1000
...
## Features desta build (v1.43)

### Novo na v1.30 — anti-freeze real do circuito (LASTCW + STALE_CHECK)
- **FIX LASTCWONNOK**: a janela de CWs por canal é agora sempre actualizada — o reenvio da última CW em NOK/timeout funciona mesmo sem `DCW MINTIME`/`CYCLE_CHECK` (antes o NOK ia sempre ao cliente)
- **FIX CACHE STATIC**: revertido para NO nos perfis (respondia com a CW velha e descartava a fresca do cartão — congelava até restart)
- **DCW STALE_CHECK**: hash novo + CW igual Ã s últimas 2 entregues = stale — segura 1x por fonte e pede outra (2Âª vez entrega)
- **DCW RETRY: 2** nos perfis de circuito + perfil **[SkyDE-098D] ICAM activado**

### Novo na v1.29 — pacote anti-"cartões marcados" (análise do MultiCS r120)
- **DCW FILTER (CWPK)**: blacklist de CWs de cartões marcados com 35 valores conhecidos; modos `AUTO` (recomendado: desligado por defeito, **ativa-se sozinho no 1Ã‚º hit** e passa a bloquear — sem ninguém ver o log), `LOGONLY` (só log) e `DROP`; regras `EXACT` (até 8 CWs por regra), `MASK` (wildcard) e `ALLEQUAL` (fake CW)

...
\t-h               this help message\n");
	...
	if (prg.restart==1) { // restart()
		...
		done_config(&cfg);
		// exec DIRETO (sem fork): mantem o mesmo PID - funciona com
		// systemd (Main PID), crontab e qualquer arranque
		char selfexe[512];
		int n = readlink("/proc/self/exe", selfexe, sizeof(selfexe)-1);
		if (n>0) selfexe[n] = 0;
		else strcpy(selfexe, argv[0]);
		// dar tempo a resposta HTTP sair (evita 502 no browser/nginx)
		sleep(2);
		// fechar todos os fds (sockets) antes do exec
		int fd;
		for (fd=3; fd<1024; fd++) close(fd);
		// rebuild argv com argv[0] absoluto
		char *newargv[64];
		int ai;
		newargv[0] = selfexe;
		for (ai=1; ai<argc && ai<63; ai++) newargv[ai] = argv[ai];
		newargv[ai] = NULL;

...
char *cc_version[] = { "2.0.11", "2.1.1", "2.1.2", "2.1.3", "2.3.0", NULL };
...
*/
		...
		if (cfg->http.autorefresh<0) cfg->http.autorefresh = 0;
				...
				parse_spaces();
				if ( parse_path(str) ) strcpy( cfg->http.title, str );
			}
		...
		// ANTICASCADE: deteccao de reshare por zapping excessivo
			...
			if (!strcmp(str,"LITE")) {
				...
				parse_spaces();
				if ( parse_path(str) ) strcpy( cfg->providers_file, str );
			}
				...
				parse_spaces();
				if ( parse_path(str) ) strcpy( cfg->ip2country_file, str );
			}
				...
				parse_spaces();
				if ( parse_path(str) ) strcpy( cfg->stylesheet_file, str );
			}
				...
				if ( (*iparser>='0')&&(*iparser<='9') ) {
					...
					while (1) {
						...
						if (!strcmp(str,"profiles")) {
							...
							parse_spaces();
							if (*iparser=='"') parse_quotes('"', str);
							else parse_value(str,"\r\n;}");
...
///////////////////////////////////////////////////////////////////////////////
			...
			if (!strcmp(str,"PORT")) {
				...
				if (!cardserver) {
					mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%d,%d): Skip CACHE SENDREQ, undefined profile\n",file->nbline,iparser-currentline);
					continue;
				}
				parse_spaces();
				if ((*iparser!=':')&&(*iparser!='=')) {
				...
				if (!cardserver) {
					mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%d,%d): Skip CACHE SENDREP, undefined profile\n",file->nbline,iparser-currentline);
					continue;
				}
				parse_spaces();
				if ((*iparser!=':')&&(*iparser!='=')) {
...
///////////////////////////////////////////////////////////////////////////////
			...
			if (cardserver) {
				...
				if ( parse_expect('{') ) { // Read Caid Providers
					...
					if (!parse_expect('}')) {
						usr->card.caid = 0;
						mlogf(LOGERROR,getdbgflag(DBG_CONFIG,0,0)," config(%s line %d,%d): '}' expected\n",file->name,file->nbline,iparser-currentline);
						continue;
					}
				}
			}
